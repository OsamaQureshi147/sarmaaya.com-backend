...
export class AssetEssentialsWrtService {
  ...
  async findAllWithoutRealTime(
    query: AssetEssentialsWithoutRealTimeEntity,
  ): Promise<AssetEssentialsWithoutRealTimeEntity[]> {
  ...
  async findOneWithoutRealTime(
    ...
    const findOneWrt =
      await this.assetEssentialsWithoutRealTimeRepository.findOne({
        where: { id: id },
      });
  ...
  async updateWithoutRealTime(
    ...
    const updateResult =
      await this.assetEssentialsWithoutRealTimeRepository.update(
        { id: id },
        dto,
      );
  ...
  async removeWithoutRealTime(id: number): Promise<{ message: string }> {
    ...
    return {
      message: `Asset Essential with id ${id} has been deleted successfully.`,
    };
  }
}
