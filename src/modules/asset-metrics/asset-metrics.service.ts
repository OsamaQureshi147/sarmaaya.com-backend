...
export class AssetMetricsService {
  ...
  async createMetric(
    assetMetricsDto: AssetMetricsDto,
  ): Promise<AssetMetricsEntity> {
    const existingMetric = await this.assetMetricsRepository.findOne({
      where: { metric: assetMetricsDto.metric },
    });
  ...
  async updateMetric(
    metric: string,
    updateData: Partial<AssetMetricsEntity>,
  ): Promise<AssetMetricsEntity> {
    const existingMetric = await this.assetMetricsRepository.findOne({
      where: { metric },
    });
  ...
  async removeMetric(metric: string): Promise<{ message: string }> {
    const metricEntity = await this.assetMetricsRepository.findOne({
      where: { metric },
    });
    ...
    const fundamentals = await this.assetFundamentalsRepository.find({
      where: { metric: metricEntity },
    });
